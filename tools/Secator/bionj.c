...
void Initialize(float **delta, FILE *input, int n, POINTERS *trees)
{
    ...
    for(lig=1; lig <= n; lig++)
    {
		retourLecture=(int)fscanf(input,"%s",name_taxon);                  /* read taxon’s name */
	  ...
	  for(col= 1; col <= n; col++)
	  {
	     retourLecture=(int)fscanf(input,"%f",&distance);             /* read the distance  */
	...
	while(input==NULL)
	  {
	    ...
	    retourLecture=(int)fgets(Name_fich1,LEN,stdin);                     /* ask for the input-file */
	    input=fopen(Name_fich1,"r");
	  }
      }
    output= fopen(fichier_sortie,"w");
    retourLecture=(int)fscanf(input,"%d",&n);
